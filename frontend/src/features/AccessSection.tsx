...
export function AccessSection() {
  ...
  return (
    <section className="mt-8">
      <h2 className="mb-1 font-display text-base font-bold">Workspace access</h2>
      <p className="mb-4 text-xs text-muted">
        Accounts with access to “{current.name}” and their roles. The team above is
        the participants added to the timeline: everyone on the team has access, but not
        everyone with access is on the team.
      </p>
                  ...
                  <td className="whitespace-nowrap px-2 py-2 text-right">
                    {!teamUserIds.has(p.user_id) && (
                      <button
                        ...
                        title="Add to the team — a row appears on the timeline"
                      >
                        to team
                      </button>
                    )}
      ...
      {isOwner && (
        <>
          <div className="mt-6 rounded-lg border border-line bg-surface p-4">
            <h2 className="text-sm font-medium">Default role</h2>
            <p className="mt-1 text-xs text-muted">
              Everyone who joins via an invite link gets this role.
            </p>
            <select
            ...
            <div className="mt-3 space-y-1.5">
              {invites.isSuccess && activeInvites.length === 0 && !createdUrl && (
                <p className="text-xs text-muted">
                  No active links — create one to invite the team.
                </p>
              )}
              {activeInvites.map((l) => (
                <div
                ...
                >
                  <div className="text-xs">
                    ...
                    <span className="ml-2 text-muted">
                      ...
                      {l.last_used_at &&
                        ` · last used ${new Date(l.last_used_at).toLocaleDateString("en-GB")}`}
                    </span>
                  </div>
      ...
      {revokingInvite && (
        <ConfirmDialog
          ...
          message={
            <>
              Revoke link <b className="font-nums">{revokingInvite.token_prefix}…</b>?
              Nobody will be able to join through it anymore; those who already joined
              keep their access.
            </>
          }
          confirmLabel="Revoke"
      ...
      {removingAccess && (
        <ConfirmDialog
          ...
          message={
            <>
              Remove access for <b>“{removingAccess.name}”</b>? They will no longer be able
              to open the workspace until they join via a link again.
            </>
          }
          confirmLabel="Remove"
